 ...
 * Available in Tauri mode and browser mode when opened from desktop
 */
export const hasAuthSupport = (): boolean => isTauri() || isOpenedFromDesktop();

/**
 * Check if market data APIs are available
 * (Only available in Tauri mode via backend)
 */
export const hasMarketDataSupport = (): boolean => isTauri();
